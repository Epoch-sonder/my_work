...
function pdQuery() {
    ...
    $query = 'INSERT INTO pd_work_test(`branch`'.$tb_complex.',`customer`,`basedoc_type`,`basedoc_name`,`basedoc_datasign`,`basedoc_datefinish`,`work_cost`,`work_datastart`,`federal_subject`,`work_name`,`work_area`,`work_othername`,`work_reason`,`forest_usage`,`comment`,`code_1C` )
                    VALUES ("'.$idBranch.'"'.$value_complex.',"'.$customerName.' ","'.$docs_type.'","'.$row[11].' ","'.$dataContract.' ","'.$dataEndings.' ","'.$row[16].'","'.$date.' ","'.$idSubject.'","'.$idType.'","'.$area.'","Иные ","1","'.$idTypeUse.'","'.$comment.'","'.$row[15].'" )';
...
// Если дата неправильная
    ...
    foreach ( $xls->rows() as $r => $row ) {
        ...
        if ($countall >= 1 and $countall <= 6) $bd_null = 1;
            ...
            $bd_null = 0;
            $query = 'SELECT * FROM   `pd_work_test` WHERE `code_1C` = "'. $row[15] .'"  LIMIT 1';
            $results = $mysqli->query($query);
...
//            }
            ...
            if ($complexOneRecord){
                $query = 'UPDATE `pd_work_test` SET `in_complex` = REPLACE(`in_complex`,"0","1") WHERE `code_1C` = "'. $row[15].'" and `in_complex` = "0"';
                $results = $mysqli->query($query);
            }
