...
if (\Yii::$app->user->can('pd_ca')) $disabled = false;
else $disabled = true;
if (Yii::$app->request->get('id') == NULL) $value = ' ';
else $value = $model->work_datastart;


if ($model->completed)$work_completed = $model->completed;
else $work_completed = 0;
...
<div <?= $model->warning == 0 ? "class='hide'" : "" ?>>
    ...
    <?php
    if (Yii::$app->request->get('id')) echo $form->field($model, 'work_othername')->textInput() ;
    else echo $form->field($model, 'work_othername')->textInput(['value'=>'Укажите наименование работ']); ?>
    ...
    <?= $form->field($model, 'comment')->textarea(['rows' => 3, 'cols' => 5 , 'disabled'=>$disabled]) //$disabled указана в начале файла ?>

    <?php //= $form->field($model, 'comment')->textInput(['maxlength' => true]) ?>
    <?php //= $form->field($model, 'timestamp')->textInput() ?>

    <?= $form->field($model, 'remark')->textarea(['rows' => 3, 'cols' => 5 ])->label('Примечания (необязательное поле, для собственных пометок)') ?>
    ...
    <?php
    if (\Yii::$app->user->can('pd_ca'))   {
        echo  $form->field($model, 'fact_datefinish')->widget(DatePicker::classname(),
            [
                'language' => 'ru-RU',
                'removeButton' => false,
                'pluginOptions' => [
